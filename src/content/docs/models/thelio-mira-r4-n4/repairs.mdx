...
3. Pull the top shell off of the chassis.
    - When reinstalling the top shell, ensure the ventilation cutouts are aligned with the radiator underneath.
...
5. Partially loosen the two back screws holding the PCIe bracket in place, and slide it into the rightmost position.

![PCIe bracket screws](./img/pcie-bracket.webp)

6. While holding the GPU, push back the latch on the motherboard (accessible from above the GPU) to free the PCIe connection, then pull the card out of the slot.

![PCIe slots & latches](./img/pcie-slots.webp)

7. Insert the new GPU (or re-seat the existing GPU) into the top PCIe slot.
8. Close the back PCIe bracket.
9. Connect the GPU power cable.
10. If necessary, adjust the position of the GPU brace fingers.
...
11. Reinstall the GPU brace, front glass, and left side panel.
...
1. Follow the steps above to [remove the left side panel](#removing-the-side-panels) and [remove the side fan dust filters and screws](#cleaning-the-side-dust-filters).
...
4. Unscrew the four screw heads holding the CPU cooler pump onto the CPU (highlighted red below).
...
5. Unplug the CPU cooler pump from the `AIO_PUMP` header on the motherboard, highlighted cyan above.
6. Unplug the combined CPU cooler fan cable from the fan splitter board next to the motherboard, highlighted green above.
7. Unfasten the velcro strip holding the CPU cooler's liquid tubes to the back of the chassis, highlighted yellow above.
8. While holding the radiator in place so it doesn't fall, unscrew the eight silver screws holding the radiator on from the top of the chassis.
...
9. Remove the CPU cooler pump and radiator (if applicable) from the chassis.
10. Using a paper towel, clean the existing thermal paste off of the cooler and CPU. You may also use a small amount of rubbing alcohol if the old paste is dried or difficult to remove.
...
**Part numbers:**
- Thelio Mira B4-N4 uses [Thelio Io version 2](https://github.com/system76/thelio-io/tree/92c2a60b6d6acc0f9d51d819cba07ad28e2b03f3) ([PCB revision thelio_io_2.3](https://github.com/system76/thelio-io-hardware/tree/91814370002f762ac01cd1dd99dcbb43336352ab/pcb-thelio-io)).
