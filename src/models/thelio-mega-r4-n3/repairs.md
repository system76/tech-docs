...
- [Replacing bottom intake fan](#replacing-the-bottom-intake-fan)
...
1. Follow the steps above to [remove the top case](#removing-the-top-case), [remove the side fan mount](#removing-the-side-fan-mount), [remove the GPU brace](#removing-the-gpu-brace), [remove the GPU brace and GPUs](#replacing-a-gpu), and [remove the M.2 heatsink](#replacing-the-m2-drives).
...
Thelio Mega R4 supports up 512GB (8x64GB) of ECC DDR5 RAM DIMMs running at a maximum speed of 5600 MHz. If you've purchased new RAM, need to replace your RAM, or are reseating your RAM, follow these steps.
...
1. Follow the steps above to [remove the top case](#removing-the-top-case), [remove the side fan mount](#removing-the-side-fan-mount), [remove the GPU brace](#removing-the-gpu-brace), and [remove the CPU duct](#removing-the-cpu-duct).
    - You can optionally [remove the CPU cooler](#replacing-the-cpu-cooler-and-cpu) for easier access to the innermost RAM slots.
...
9. If the replacement PSU has a "Smart Zero Fan" switch, make sure it's switched on.
...
1. When wiring the Thelio Io board, refer to the port labels and the following guide.
    ...
    - `FANOUT4`/`AUX FAN` - to the right fan splitter (connecting to the [duct-mounted CPU fans](#replacing-the-cpu-fans)).
    ...
    - `USB` - to the `F_USB2` header on the bottom edge of the motherboard, highlighted yellow above.
        ...
        - This port can alternatively be connected to the `F_USB1` header if the `F_USB2` header becomes defective.
...
6. Plug the 4-pin Berg power connector and SATA data cables into the SATA backplane.
    ...
    - By default, the ports connect as follows:
        - Top SATA backplane (top row of motherboard ports): `DATA0` connects to motherboard port 1. `DATA1` connects to motherboard ort 0.
    ...
    - The SATA ports can be accessed from behind via the cabling cutout. In the below photo, the GPU brace, CPU shroud, and GPU have been removed to show the ports from the front.
...
9. Replace the side brace (if necessary), CPU shroud, and top case.
...
3. Push the small button labeled `POWER` on the top Thelio-IO board.
...
5. If the Thelio-IO `POWER` button does not work, press the `Power` button in the top right of the motherboard.
