# Oryx Pro (External Overview)

## Left side:

<span class="text-on-transparency">![Left Ports](./img/ports-left.webp)</span>

## Right side:

<span class="text-on-transparency">![Right Ports](./img/ports-right.webp)</span>

## Front side:

<span class="text-on-transparency">![Front Ports](./img/ports-front.webp)</span>

## Back side:

<span class="text-on-transparency">![Back Ports](./img/ports-back.webp)</span>

## Keyboard & touchpad:

![Keyboard & Touchpad](./img/keyboard-touchpad.webp)

## Lid:

![Lid](./img/lid.webp)

## Materials:

|Part                              |Material                |Part Number    |
|----------------------------------|------------------------|---------------|
|LCD back cover (lid)              |Aluminum                |6-39-V5601-012 |
|LCD front cover (bezel)           |Plastic                 |6-39-V5601-013 |
|Top case (palm rests/port covers) |Plastic                 |6-39-V56K2-010 |
|Bottom panel                      |Aluminum                |6-39-V56K3-012 |

## LED indicators:

|Icon                                    |Color          |Description                      |
|----------------------------------------|---------------|---------------------------------|
|![Power LED](./img/led-power.avif)      |Orange         |Powered off, DC power plugged in |
|                                        |Green          |Powered on                       |
|                                        |Blinking green |Sleeping (suspended)             |
|![Battery LED](./img/led-battery.avif)  |Orange         |Battery charging                 |
|                                        |Green          |Battery fully charged            |
|                                        |Blinking orange|Battery critically low           |

## Keyboard shortcuts:

|Key                          |Shortcut|Action                           |
|-----------------------------|--------|---------------------------------|
|![Fn-F1](./img/fn-f1.webp)   |Fn+F1   |Toggle trackpad                  |
|![Fn-F2](./img/fn-f2.webp)   |Fn+F2   |Mute/unmute microphone           |
|![Fn-F3](./img/fn-f3.webp)   |Fn+F3   |Mute/unmute speakers             |
|![Fn-F5](./img/fn-f5.webp)   |Fn+F5   |Volume down                      |
|![Fn-F6](./img/fn-f6.webp)   |Fn+F6   |Volume up                        |
|![Fn-F7](./img/fn-f7.webp)   |Fn+F7   |Toggle displays                  |
|![Fn-F8](./img/fn-f8.webp)   |Fn+F8   |Screen brightness down           |
|![Fn-F9](./img/fn-f9.webp)   |Fn+F9   |Screen brightness up             |
|![Fn-F10](./img/fn-f10.webp) |Fn+F10  |Toggle webcam                    |
|![Fn-F11](./img/fn-f11.webp) |Fn+F11  |Toggle airplane mode             |
|![Fn-F12](./img/fn-f12.webp) |Fn+F12  |Suspend                          |
|![Fn-F12](./img/fn-dia.avif) |Fn+`    |Play/Pause                       |
|![Fn-*](./img/fn-star.webp)  |Fn+*    |Toggle keyboard backlight        |
|![Fn-/](./img/fn-slash.webp) |Fn+/    |Cycle keyboard color             |
|![Fn--](./img/fn-minus.webp) |Fn+-    |Decrease keyboard brightness     |
|![Fn-+](./img/fn-plus.webp)  |Fn++    |Increase keyboard brightness     |
|1                            |Fn+1    |Toggle fan between max/automatic |
